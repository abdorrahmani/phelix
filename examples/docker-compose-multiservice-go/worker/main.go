// Command worker-demo is the second independent Go service in the multiservice
// example — its own Phelix app, its own public port (8090), sharing the network
// and Redis with api-demo. Standard library only.
package main
...
func main() {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprintf(w, "hello from worker-demo on port %s\n", port)
	})
	...
	addr := ":" + port
	log.Printf("worker-demo listening on %s (redis at %s)", addr, redisAddr)
