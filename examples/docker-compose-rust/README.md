...
A minimal Rust app that Phelix runs **as a container on a shared Docker
network**, beside a backing service (Redis) owned by **docker-compose** — the
**split-ownership** model of the
[Docker runtime](../../docs/guides/docker-runtime.md).

- **Phelix owns the app tier:** it builds `compose-rust-demo:vN` and
...
> [`../docker-rust`](../docker-rust/). Demonstrates the mechanism; not
> production-ready. Standard library only — the Redis check is a raw RESP `PING`
> over TCP, no crate.

## What it demonstrates

- `deploy.runtime: docker` + `deploy.network: phelix-compose-rust` — the app
...
- `Dockerfile` / `.dockerignore` are exactly what **`phelix dockerize`
  generates** for a Rust project (multi-stage → `debian:bookworm-slim`).

## Run it

```bash
cd examples/docker-compose-rust

# 1. Backing tier: creates the "phelix-compose-rust" network AND starts Redis.
docker compose up -d
...
phelix build compose-rust-demo  # first deploy: image → container on the network → health → proxy

# 3. Verify — the app reached Redis across the shared network.
curl localhost:8080/            # hello from compose-rust-demo on port 8080
...
```bash
phelix rebuild compose-rust-demo    # builds :v2, health-checks it, switches the proxy
phelix rollback compose-rust-demo   # back to the previous image, zero downtime
```
...
```bash
phelix dockerize compose-rust-demo --tag latest
```
