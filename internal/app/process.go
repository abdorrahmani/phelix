...
func (m *AppManager) startApplicationProcess(id string, name string, port int, logFile string) error {
	// The log file handle must stay open for as long as the child lives: it is
	// the child's stdout/stderr sink. Closing it when this function returns
	// would orphan any in-flight output. It is released either when this CLI
	// process exits (kernel closes all fds) or by waitAndCloseLog when the
	// monitor daemon reaps an exited child.
